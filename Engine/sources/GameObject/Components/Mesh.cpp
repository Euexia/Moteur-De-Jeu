...
namespace std
{
	...
	{
		size_t operator()(const Mesh::Vertex& _vertex) const
		{
			size_t seed = 0;
	...
	Mesh::Mesh(lve::LveDevice& _lveDevice, const Builder& _builder) : lveDevice(_lveDevice)
	{
	...
	std::unique_ptr<Mesh> Mesh::CreateModelFromFile(lve::LveDevice& _device, const std::string& _filepath)
	{
		Builder builder{};
		builder.LoadModel(ENGINE_DIR + _filepath);
		std::cout << "Vertex Count" << builder.vertices.size() << std::endl;

		return std::make_unique<Mesh>(_device, builder);
	}


	void Mesh::Bind(const vk::CommandBuffer _commandBuffer) const
	{
	...
	void Mesh::Draw(const vk::CommandBuffer _commandBuffer) const
	{
	...
	void Mesh::CreateVertexBuffer(const std::vector<Vertex>& _vertices)
	{
	...
	void Mesh::CreateIndexBuffer(const std::vector<uint32_t>& _indices)
	{
	...
	void Mesh::Builder::LoadModel(const std::string& _filepath)
	{
