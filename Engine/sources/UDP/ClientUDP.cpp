...
void ClientUDP::onMessageReceived(std::unique_ptr<Bousk::Messages::Base>&& msg)
{
...
void ClientUDP::sendTo(SOCKET _mSocket, std::vector<uint8_t>&& data)
{
}
...
{
	auto itClient = std::find_if(mClients.begin(), mClients.end(), [&](const std::unique_ptr<DistantClient>& client)
		{
...
std::vector<std::unique_ptr<Bousk::Messages::Base>> ClientUDP::poll()
{
	return std::vector<std::unique_ptr<Bousk::Messages::Base>>();
}
